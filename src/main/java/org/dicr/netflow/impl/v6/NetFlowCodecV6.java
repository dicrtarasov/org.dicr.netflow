/**
 * NetFlowCodecV6.java 20.12.2006
 */
 ...
 * @version 070112
 */
public final class NetFlowCodecV6 extends NetFlowCodec {
	/** Singleton instance */
	public static final NetFlowCodecV6 INSTANCE = new NetFlowCodecV6();

	/** Private constructor */
	private NetFlowCodecV6() {
		super();
	}

	/** Encode flow */
	private static void encodeFlow(FlowV6 flow, ByteBuffer buf) throws CodecException {
	...
	/** Decode flow */
	private static FlowV6 decodeFlow(ByteBuffer buf) throws CodecException {
		if (buf == null) throw new IllegalArgumentException("null buf");
		FlowV6 flow = new FlowV6();
		try {
			...
			buf.getShort(); // skip 2 unused bytes
			buf.getInt(); // skip 4 unused bytes
	...
	public void encodePacket(NetFlowPacket packet, ByteBuffer buf) throws CodecException {
		...
		if (!(packet instanceof NetFlowPacketV6)) throw new IllegalArgumentException("incorrect packet type: "
		        + packet.getClass());
		if (buf == null) throw new IllegalArgumentException("null buffer");
		NetFlowPacketV6 packetv6 = (NetFlowPacketV6) packet;
		try {
			buf.putShort((short) FlowTypeV6.VERSION);
			buf.putShort((short) packetv6.getFlowsCount());
			buf.putInt((int) packetv6.getSysUptime());
			buf.putInt((int) packetv6.getUnixSecs());
			buf.putInt((int) packetv6.getUnixNSecs());
			buf.putInt((int) packetv6.getFlowSequence());
			buf.put((byte) packetv6.getEngineType());
			buf.put((byte) packetv6.getEngineId());
			buf.putShort((short) packetv6.getSamplingInterval());
			Collection<Flow> flows = packetv6.getFlows();
			for (Flow flow : flows) {
				encodeFlow((FlowV6) flow, buf);
			}
		} catch (BufferOverflowException ex) {
			throw new CodecException("data buffer overflow", ex);
		}
	 ...
	 */
	@Override
	public NetFlowPacketV6 decodePacket(ByteBuffer buf) throws CodecException {
		if (buf == null) throw new IllegalArgumentException("null buffer");
		NetFlowPacketV6 packet = new NetFlowPacketV6();
		try {
			// version
			int versionCode = ByteUtils.unsigned(buf.getShort());
			if (versionCode != FlowTypeV6.VERSION) throw new UnsupportedVersionException("packet version "
			        + versionCode + " does not match required version: " + FlowTypeV6.VERSION);

			int count = ByteUtils.unsigned(buf.getShort());
			if (count < 0 || count > FlowTypeV6.MAX_FLOWS_COUNT) throw new NetFlowException("incorrect flows count: "
			        + count);
			...
			packet.setSamplingInterval(ByteUtils.unsigned(buf.getShort()));
